  ...
  ngOnInit(): void {
    ...
    try {
      this.productoTipoService.getProductoTipo(this.id).subscribe({
        ...
        error: err => {
          this.msg.error(formatErrorMsg(this.i18n.getI18Value('services.product_type.individual.get.error'), err));
        }
      });
    } catch (err: any) {
      this.msg.error(formatErrorMsg(this.i18n.getI18Value('form.product_type.individual.get.error'), err));
    }
  }
  ...
  submit(productoTipo: IProductoTipo) {
    try {
      ...
      this.productoTipoService.updateProductoTipo({ ...update_data, id: this.id! }).subscribe({
        ...
        error: err => {
          this.msg.error(formatErrorMsg(this.i18n.getI18Value('services.product_type.update.error'), err));
        }
      });
    } catch (err: any) {
      this.msg.error(formatErrorMsg(this.i18n.getI18Value('form.product_type.individual.update.error'), err));
    }
  }
}
